...
Run scripts as a normal user. Do **not** use `sudo bash scripts/...`; the service script invokes `sudo` internally for apt, systemctl, and Neo4j-owned paths, so the terminal will ask for your sudo password when needed.

### 1. Prepare conda environments and `.env` files
...
This sets up backend/frontend conda environments, installs Python dependencies, installs local backend DGL-KE, installs backend Poetry dependencies, and creates `Backend/.env` / `Frontend/.env` from examples when missing.

Expected time on a fresh machine:

- backend dependencies: about 20-75+ minutes
- frontend dependencies: about 5-25 minutes

The script prints numbered stages, estimated time per stage, start time, final duration, and pip download progress. Re-running is usually faster because conda/pip caches are reused.

First-run verification warnings are expected at this point because Neo4j, Redis, model paths, JWT secret, and LLM/API settings may not be configured yet. The setup helper reports those as concise warnings and prints the next steps.

### 2. Download the Neo4j graph dump

The EvoAge Neo4j dump is hosted here:
...
The dump is large, so running this in a separate terminal from the EvoAge repo root is recommended:

```bash
bash scripts/download_neo4j_dump.sh
```

The script saves the tarball and extracts the Neo4j dump here:
...
If the tarball is already downloaded, rerunning the script reuses the existing file. If `data/neo4j/neo4j.dump` is missing, it extracts the dump from the tarball automatically.

### 3. Fill service values, install services, enable APOC, and restore Neo4j

After the dump is downloaded/extracted, fill the service values in `Backend/.env`:
...
- installs Java, Neo4j, Redis, and Redis CLI tooling on Debian/Ubuntu
- reads Neo4j/Redis settings from `Backend/.env`
- uses the default extracted dump path: `data/neo4j/neo4j.dump`
- sets Redis password and restarts Redis
- stops Neo4j before database restore
- sets the Neo4j password when the DBMS has not been initialized
- downloads and enables APOC
- restores the dump into the `neo4j` database
- restarts Neo4j after APOC configuration and dump restore
- checks Redis `PING`
- checks Neo4j login with `SHOW DATABASES`
- checks graph availability with `MATCH (n) RETURN count(n) AS nodeCount`
- syncs service connection values into `Backend/.env` and `Frontend/.env`

### 4. Fill remaining env values and run setup checks

Fill remaining required backend values before final verification, especially KGE model paths, hypothesis-testing paths, JWT secret, and LLM settings.

```bash
bash scripts/setup.sh --check-only
```

The final check validates required `.env` values, prints the backend/frontend URLs, checks Redis/Neo4j, and tries to reach the backend/frontend URLs if those servers are running.

`--check-only` does not start the backend or frontend. It is a verification command only.
...
The launcher starts both processes in the background, writes logs/PID files, prints URLs, and checks whether the URLs become reachable.

Default URLs:

- backend API: `http://localhost:1026`
- backend Swagger docs: `http://localhost:1026/docs`
...
Manual setup remains documented below for environments where package installation must be controlled by an administrator.
